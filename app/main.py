...
def scenario_content(scenario_id: str, refresh: bool = False):
    scenario = get_scenario(scenario_id)
    if not scenario:
        raise HTTPException(404, "シナリオが見つかりません。")
    if refresh:
        db.clear_cached_content(scenario_id)
    content = None if refresh else db.get_cached_content(scenario_id)
    if content is None:
        content = gemini.generate_scenario_content(scenario)
        db.set_cached_content(scenario_id, content)
...
def chat(req: ChatRequest):
    ...
    if req.scenario_id:
        ...
        if not scenario:
            raise HTTPException(404, "シナリオが見つかりません。")
        reply = gemini.roleplay_reply(scenario, messages)
    else:
        reply = gemini.free_chat_reply(messages)
    return {"reply": reply}
...
def feedback(req: FeedbackRequest):
    ...
    return gemini.generate_feedback(title, [m.model_dump() for m in req.messages])


@app.post("/api/sessions")
def record_session(req: SessionRequest):
    db.add_session(req.scenario_id, req.mode, req.score, req.feedback)
